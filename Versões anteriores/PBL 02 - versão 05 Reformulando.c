...
//====================VARIÁVEIS USADAS NA FUNÇÃO PRINCIPAL===========================	
	...
	int pedirNovoArquivo = 0, devePedir = 0, controle = FALSE, idenDoPolitico[MAX];
...
//=============INICIO DAS INSTRUÇÕES=============	
	do{
		...
		controle = verificaSeVazio(nomeArquivo);
		if(controle == TRUE){
			devePedir = erroMsn();
			if (devePedir == TRUE)
			...
			controle = verificaLinha(nomeArquivo);
			if(controle == TRUE){
				devePedir = erroMsn();
				if (devePedir = TRUE)
					pedirNovoArquivo = TRUE;
				...
				controle = verificaColuna(nomeArquivo,JNDPE);
					if (controle == TRUE){
						devePedir = erroMsn();
						if(devePedir == TRUE)
							pedirNovoArquivo = TRUE;
