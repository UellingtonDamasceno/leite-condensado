...
//====================VARIÁVEIS USADAS NA FUNÇÃO PRINCIPAL===========================	
	...
	float totalProprina[MAX], mediaPoli[MAX][MAX], mediaEmpre[MAX][MAX];//armazena a media dos valores pagos/recebidos;
			...
			if(tratarErro == TRUE){
				...
				tratarErro = verificaColuna(nomeArquivo, JNDPE);
					if (tratarErro == TRUE){
						...
						totalPagoPorEmpresa(proprinaCJ, JNDPE, EtotalPagoCJ, proprina, EtotalPagoSJ);
						maisEmenos(proprinaCJ, proprinaSJ, PmaisEmenosDin ,JNDPE, 1);
						//maisEmenos(EtotalPagoCJ, EtotalPagoSJ, EmaisEmenosDin ,JNDPE , 2);
						/*
...
//======================================================================================================================================================================================
void calculaProprina(float proprinaCalculada[MAX][MAX],
					 float jurosCalculado[MAX][MAX], float proprina[MAX][MAX], float JNDPE[2], float proprinaSJ[MAX], float proprinaCJ[MAX]){
...
void totalPagoPorEmpresa(float proprinaCalculada[MAX][MAX], float JNDPE[2], float EtotalPagoCJ[MAX], float proprina[MAX][MAX], float EtotalPagoSJ[MAX]){
	...
	for(empresa = 0; empresa < JNDPE[2]; empresa++){
		...
		somaSJ = 0;
		for(politico = 0; politico < JNDPE[1]; politico++){	
			somaCJ += proprinaCalculada[politico][empresa];//calcula a proprina com o juros.
...
void maisEmenos(float PCJ[MAX], float PSJ[MAX], float maisEmenos[7], float JNDPE[2], int PouE){
	int i;
	maisEmenos[2] = MIN_LIMITE;
	maisEmenos[5] = MAX_LIMITE;
